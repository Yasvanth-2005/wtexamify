...
import { ArrowLeft, Trash2, PlusCircle, Loader } from 'lucide-react';
...
const EditExam = () => {
  ...
  const [saveLoading, setSaveLoading] = useState(false);   // form submit
  ...
  const fetchExamData = async () => {
    try {
      ...
      // result.exam has { name, duration, exam_type, questions[] }
  ...
  // Full-page loader while fetching exam data
  if (pageLoading) {
    return (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Upload New Questions (Excel file — single column)
              </label>
              <input
                type="file"
                accept=".xlsx,.xls"
              ...
              />
              <p className="mt-1 text-xs text-gray-500">Uploaded questions will be appended to the existing list.</p>
            </div>
            ...
            <div>
              ...
              {examData.questions.length === 0 ? (
                <p className="text-sm text-gray-500 italic">No questions yet. Upload an Excel file or add manually above.</p>
              ) : (
